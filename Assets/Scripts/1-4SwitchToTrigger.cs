...
public class SwitchMoveToUp : MonoBehaviour
{
    ...
    public float moveDistance = 5f; // 이동 거리 (우측으로 10칸)
    ...
    void Start()
    {
        ...
        // "Move" 태그를 가진 발판 찾기
        GameObject movePlatform = GameObject.FindGameObjectWithTag("trick");
        if (movePlatform != null)
        {
            movePlatformTransform = movePlatform.transform;
            targetPosition = movePlatformTransform.position + new Vector3(0, moveDistance, 0); // 우측으로 5칸 이동
        }
        else
        {
